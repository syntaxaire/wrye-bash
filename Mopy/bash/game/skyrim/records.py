...
class MreRace(MelRecord):
    """Race"""
    classType = 'RACE'
    RaceFlags1 = Flags(0L, Flags.getNames('playable', 'faceGenHead', 'child',
                                          'tiltFrontBack', 'tiltLeftRight',
                                          'noShadow', 'swims', 'flies',
                                          'walks', 'immobile', 'notPushable',
                                          'noCombatInWater',
                                          ...
                                          'regenHPInCombat', 'cantOpenDoors',
                                          'allowPCDialogue', 'noKnockdowns',
                                          'allowPickpocket',
                                          ...
                                          'canDualWield', 'avoidsRoads'))
    RaceFlags2 = Flags(0L, Flags.getNames('useAdvancedAvoidance', 'nonHostile',
                                          'unknown1', 'unknown2',
                                          'allowMountedCombat'))
    RaceEquipmentFlags = Flags(0L, Flags.getNames('handToHandMelee',
                                                  'oneHandSword',
                                                  'oneHandDagger',
                                                  'oneHandAxe',
                                                  'oneHandMace',
                                                  'twoHandSword',
                                                  'twoHandAxe', 'bow', 'staff',
                                                  'spell', 'shield', 'torch',
                                                  'crossbow'))
    ...
    melSet = MelSet(
        ...
        MelLString('DESC', 'description'),
        MelCountedFids('SPLO', 'spells', 'SPCT'),
        MelOptStruct('WNAM', 'I', (FID, 'skin')),
        MelBipedObjectData(),
        MelKeywords(),
        MelStruct('DATA', '14b2s4fI7fI2ifi5fi4fI9f', 'skill1', 'skillBoost1',
                  'skill2', 'skill2Boost', 'skill3', 'skill3Boost', 'skill4',
                  'skill4Boost', 'skill5', 'skill5Boost', 'skill6',
                  'skill6Boost', 'skill7', 'skill7Boost', ('unknown1', null2),
                  'maleHeight', 'femaleHeight', 'maleWeight', 'femaleWeight',
                  (RaceFlags1, 'flags', 0L), 'startingHealth',
                  'startingMagicka', 'startingStamina', 'baseCarryWeight',
                  'baseMass', 'accelerationRate', 'decelerationRate',
                  'raceSize', 'headBipedObject', 'hairBipedObject',
                  'injuredHealthPercentage', 'shieldBipedObject',
                  'healthRegen', 'magickaRegen', 'staminaRegen',
                  'unarmedDamage', 'unarmedReach', 'bodyBipedObject',
                  'aimAngleTolerance', 'flightRadius',
                  'angularAccelerationRate', 'angularTolerance',
                  (RaceFlags2, 'flags2', 0L), ('mountDataOffsetX', -63.479000),
                  'mountDataOffsetY', 'mountDataUnknown1',
                  ('mountDataUnknown2', -50.0), 'mountDataUnknown3',
                  ('mountDataUnknown4', 65.0), 'mountDataUnknown5',
                  ('mountDataUnknown6', -300.0), 'mountDataUnknown7'),
        # {--- Skeletons ---}
        MelBase('MNAM', 'maleMarker'),
        MelString('ANAM', 'maleSkeletalModel'),
        # Ignore texture hashes - they're only an optimization, plenty of
        # records in Skyrim.esm are missing them
        MelNull('MODT'),
        MelBase('FNAM', 'femaleMarker'),
        MelString('ANAM', 'femaleSkeletalModel'),
        # Ignore texture hashes - they're only an optimization, plenty of
        # records in Skyrim.esm are missing them
        MelNull('MODT'),
        # {--- Misc #1 ---}
        MelBase('NAM2', '_nam2_1'),
        MelGroups('movementTypeNames',
            MelString('MTNM', 'movementType'),
        ),
        MelStruct('VTCK', '2I', (FID, 'maleVoice'), (FID, 'femaleVoice')),
        MelOptStruct('DNAM', '2I', (FID, 'decapitateArmorMale'),
                     (FID, 'decapacitateArmorFemale')),
        MelOptStruct('HCLF', '2I', (FID, 'defaultHairColorMale'),
                     (FID, 'defaultHairColorFemale')),
        # Theoretically, this needs to be auto-updated to match the number of
        # tints in the tint list further down, but not even xEdit does this
        # right now.
        MelOptStruct('TINL', 'H', 'totalTintCount'),
        MelStruct('PNAM', 'f', 'faceGenMainClamp'),
        MelStruct('UNAM', 'f', 'faceGenFaceClamp'),
        MelOptStruct('ATKR', 'I', (FID, 'attackRace')),
        MelGroups('attacks',
            MelAttackData(),
            MelString('ATKE', 'attackEvents'),
        ...
        # {--- Body Data ---}
        MelBase('NAM1', 'bodyDataMarker'),
        MelBase('MNAM', 'maleBodyDataMarker'),
        MelGroups('maleBodyData',
            MelStruct('INDX', 'I', 'index'),
            MelModel(),
        ),
        MelBase('FNAM', 'femaleBodyDataMarker'),
        MelGroups('femaleBodyData',
            MelStruct('INDX', 'I', 'index'),
            MelModel(),
        ),
        MelFidList('HNAM', 'raceHairs'),
        MelFidList('ENAM', 'raceEyes'),
        MelFid('GNAM', 'bodyPartData'),
        # {--- Behavior ---}
        MelBase('NAM2', '_nam2_2'),
        MelBase('NAM3', 'hkxMarker'),
        MelBase('MNAM', 'maleBehaviorGraphMarker'),
        MelModel('maleBehaviorGraph'),
        MelBase('FNAM', 'femaleBehaviorGraphMarker'),
        MelModel('femaleBehaviorGraph'),
        # {--- Misc #2 ---}
        MelOptStruct('NAM4', 'I', (FID, 'materialType')),
        MelOptStruct('NAM5', 'I', (FID, 'impactDataSet')),
        MelOptStruct('NAM7', 'I', (FID, 'decapitationFX')),
        MelOptStruct('ONAM', 'I', (FID, 'openLootSound')),
        MelOptStruct('LNAM', 'I', (FID, 'closeLootSound')),
        MelGroups('bipedObjectNames',
            MelString('NAME', 'name'),
        ),
        MelGroups('movementTypes',
            MelOptStruct('MTYP', 'I', (FID, 'movementType')),
            MelOptStruct('SPED', '11f', 'leftWalk', 'leftRun', 'rightWalk',
                         'rightRun', 'forwardWalk', 'forwardRun',
                         'backwardWalk', 'backwardRun', 'rotateWalk',
                         'rotateRun', 'unknown1'),
        ),
        # {--- Equipment ---}
        MelOptStruct('VNAM', 'I', (RaceEquipmentFlags, 'equipmentFlags', 0L)),
        MelFids('QNAM', 'equipSlots'),
        MelFid('UNES', 'unarmedEquipSlot'),
        MelGroups('phonemeTargetNames',
            MelString('PHTN', 'name'),
        ),
        # {--- Phonemes ---}
        MelGroups('faceFXPhonemes',
            # If this turns out to be too expensive, just replace with MelBase
            MelStruct('PHWT', '16f', 'Aah_LipBigAah_weight',
                      'BigAah_LipDST_weight', 'BMP_LipEee_weight',
                      'ChJsh_LipFV_weight', 'DST_LipK_weight',
                      'Eee_LipL_weight', 'Eh_LipR_weight', 'FV_LipTh_weight',
                      'I_weight', 'K_weight', 'N_weight', 'Oh_weight',
                      'OohQ_weight', 'R_weight', 'TH_weight', 'W_weight'),
        ),
        # {--- Movement Defaults ---}
        MelFid('WKMV', 'baseMovementDefaultWalk'),
        MelFid('RNMV', 'baseMovementDefaultRun'),
        MelFid('SWMV', 'baseMovementDefaultSwim'),
        MelFid('FLMV', 'baseMovementDefaultFly'),
        MelFid('SNMV', 'baseMovementDefaultSneak'),
        MelFid('SPMV', 'baseMovementDefaultSprint'),
        # {--- Male Head Data ---}
        MelBase('NAM0', 'headDataMarker1'),
        MelBase('MNAM', 'maleHeadDataMarker'),
        MelGroups('maleHeadParts',
            MelStruct('INDX', 'I', 'index'),
            MelFid('HEAD', 'head'),
        ),
        # All MPAVs are decoded-ish in xEdit, but including them seems wholly
        # unnecessary
        MelBase('MPAI', 'unknown2'),
        MelBase('MPAV', 'maleNoseVariants'),
        MelBase('MPAI', 'unknown3'),
        MelBase('MPAV', 'maleBrowVariants'),
        MelBase('MPAI', 'unknown4'),
        MelBase('MPAV', 'maleEyeVariants'),
        MelBase('MPAI', 'unknown5'),
        MelBase('MPAV', 'maleLipVariants'),
        MelFids('RPRM', 'maleRacePresets'),
        MelFids('AHCM', 'maleHairColors'),
        MelFids('FTSM', 'maleFaceDetailsTextureSets'),
        MelOptStruct('DFTM', 'I', (FID, 'maleDefaultFaceTexture')),
        MelGroups('maleTintMasks',
            MelGroups('textures',
                MelStruct('TINI', 'H', 'index'),
                MelString('TINT', 'fileName'),
                MelOptStruct('TINP', 'H', 'maskType'),
                MelOptStruct('TIND', 'I', (FID, 'presetDefault')),
            ),
            MelGroups('presets',
                MelFid('TINC', 'color'),
                MelStruct('TINV', 'f', 'defaultValue'),
                MelStruct('TIRS', 'H', 'index'),
            )
        ),
        MelModel('maleHeadModel'),
        # {--- Female Head Data ---}
        MelBase('NAM0', 'headDataMarker2'),
        MelBase('FNAM', 'femaleHeadDataMarker'),
        MelGroups('femaleHeadParts',
            MelStruct('INDX', 'I', 'index'),
            MelFid('HEAD', 'head'),
        ),
        # All MPAVs are decoded-ish in xEdit, but including them seems wholly
        # unnecessary
        MelBase('MPAI', 'unknown6'),
        MelBase('MPAV', 'femaleNoseVariants'),
        MelBase('MPAI', 'unknown7'),
        MelBase('MPAV', 'femaleBrowVariants'),
        MelBase('MPAI', 'unknown8'),
        MelBase('MPAV', 'femaleEyeVariants'),
        MelBase('MPAI', 'unknown9'),
        MelBase('MPAV', 'femaleLipVariants'),
        MelFids('RPRF', 'femaleRacePresets'),
        MelFids('AHCF', 'femaleHairColors'),
        MelFids('FTSF', 'femaleFaceDetailsTextureSets'),
        MelOptStruct('DFTF', 'I', (FID, 'femaleDefaultFaceTexture')),
        MelGroups('femaleTintMasks',
            MelGroups('textures',
                MelStruct('TINI', 'H', 'index'),
                MelString('TINT', 'fileName'),
                MelOptStruct('TINP', 'H', 'maskType'),
