...
There are three main differences between this system and wx's:
  ...
  2. Instead of passing wx's event classes directly to listeners, we apply
     'processors' to them beforehand. A good example is the text_processor
     below. When a EVT_TEXT (i.e. text changed) event is triggered by wx, this
     processor can be used to send the new text instead of the event itself to
     subscribers.
...
Subscribing:
  ...
  instance provided by the GUI component you're using. For example:
    ...
    >>> from gui.text_components import TextArea
...
Creating Event Handlers:
  ...
  when a component is resized).
    ...
    2. Decide what processor is needed. By default,
       gui.events.null_processor is used, which will simply discard the
       event object. In this case, we can come up with a useful processor:
       ...
       unpacked via Python's * operator and fed to the listeners. If the event
       object itself is needed for some reason, you can use
       gui.events.identity_processor to pass the event object directly to the
       listener.
...
# Predefined argument processors
def identity_processor(event):
    """Argument processor that simply returns the event itself."""
    return [event]
...
def text_processor(event):
    """Argument processor for CommandEvent subclasses that care about some
    string that changed (e.g. EVT_TEXT)."""
    return [event.GetString()]

# should become an enum in py3
...
class EventHandler(object):
    ...
    listeners."""
    def __init__(self, wx_owner, wx_event_id, arg_processor=null_processor):
    ...
    def _post(self, event):
        ...
        result to all listeners that have subscribed to this event handler.
        Also checks the return value of the listener and reacts accordingly.
    ...
    def subscribe(self, listener):
        """Subscribes the specified listener to this event handler.
    ...
    def _update_wx_binding(self):
        ...
        system."""
        # Ordered to cater to the most common case in which both ifs are False
        # (_is_bound is True and _listeners is nonempty)
